$(function(){
    ...
    var logRoleUpViewModel = function () {
        this.dataList               = ko.observableArray();
        this.accountId              = ko.observable("");
        this.roleId                 = ko.observable("");
        this.getServerIdChange      = ko.computed(function () {
            if (indexModel.zeusidChange() == "" && $(".pageContainer").attr("data-url") == "/static/pages-html/log-sum/log-role/log-roleUp.html") {
    ...
    var newLogModel = new logRoleUpViewModel();
    ko.applyBindings(newLogModel, $("#roleLogUp").get(0));
    ...
    var pageClick = function (pageclickednumber) {
        ...
        $.ajax({
            type: "get",
            async: true,
            url: "/gameLog/getPlayerLevelUpLogPage",
            data: jsonData,
            dataType: "json",
            error: function () {
                layer.close(index);
            ...
            success: function (data) {
                console.log("角色升级日志:",data)
                layer.close(index);
                if (data.code == 200) {
                    var tabDom = $("#rolelogUp_tbody").empty();
                    ...
                    if (data.info.rows) {
                        ...
                        console.log("log-roleUp data.info.rows is no data.");
                    }
    ...
    var roleForm = $("#logRoleUpForm").Validform({
