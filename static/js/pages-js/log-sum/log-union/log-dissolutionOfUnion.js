$(function(){
	...
	var logCreateUnionViewModel = function(){
		this.dataList				= ko.observableArray();
		this.leaderId				= ko.observable("");
		this.guildId				= ko.observable("");
		this.getServerIdChange		= ko.computed(function(){
			if (indexModel.zeusidChange() == "" && $(".pageContainer").attr("data-url") == "/static/pages-html/log-sum/log-union/log-dissolutionOfUnion.html"){
	...
	var newLogModel = new logCreateUnionViewModel();
	ko.applyBindings(newLogModel,document.getElementById("dissolutionOfUnion"));
	...
	var pageClick = function( pageclickednumber ){
		...
		newLogModel.leaderId() ? $.extend(jsonData,{playerId: newLogModel.leaderId()}) : $.extend(jsonData,{});
		$.ajax({
			type : "get",
			async:true,
			url:"",
			...
			success : function(data){
				console.log("解散公会日志：",data)
				layer.close(index); 
				if( data.code == 200 ){
					var tabDom = $("#dissolutionOfUnion_tbody").empty();
					...
					if (data.info.rows) {
						...
						console.log("log-dissolutionOfUnion data.info.rows is no data.");
					}
	...
	var roleForm = $("#dissolutionOfUnionForm").Validform({
