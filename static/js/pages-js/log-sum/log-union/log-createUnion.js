$(function(){
	...
	var logDissolutionOfUnionViewModel = function(){
		this.dataList				= ko.observableArray();
		this.leaderId				= ko.observable("");
		this.guildId				= ko.observable("");
		this.getServerIdChange		= ko.computed(function(){
			if (indexModel.zeusidChange() == "" && $(".pageContainer").attr("data-url") == "/static/pages-html/log-sum/log-union/log-createUnion.html"){
	...
	var newLogModel = new logDissolutionOfUnionViewModel();
	ko.applyBindings(newLogModel,$("#createUnion").get(0));
	...
	var pageClick = function( pageclickednumber ){
		...
		$.ajax({
			...
			success : function(data){
				console.log("创建公会日志：",data)
				layer.close(index); 
				if( data.code == 200 ){
					var tabDom = $("#createUnione_tbody").empty();
					...
					if (data.info.rows) {
						...
						console.log("log-createUnion data.info.rows is no data.");
					}
	...
	var roleForm = $("#createUnionForm").Validform({
