$(function(){
	...
	var logJoinUnionViewModel = function(){
		this.dataList				= ko.observableArray();
		this.roleId					= ko.observable("");
		this.guildId				= ko.observable("");
		this.getServerIdChange		= ko.computed(function(){
			if (indexModel.zeusidChange() == "" && $(".pageContainer").attr("data-url") == "/static/pages-html/log-sum/log-union/log-joinUnion.html"){
	...
	var newLogModel = new logJoinUnionViewModel();
	ko.applyBindings(newLogModel,document.getElementById("joinUnion"));
	...
	var pageClick = function( pageclickednumber ){
		...
		$.ajax({
			...
			success : function(data){
				console.log("加入公会日志：",data)
				layer.close(index); 
				if( data.code == 200 ){
					var tabDom = $("#joinUnion_tbody").empty();
					...
					if (data.info.rows) {
						...
						console.log("log-joinUnion data.info.rows is no data.");
					}
	...
	var roleForm = $("#joinUnionForm").Validform({
