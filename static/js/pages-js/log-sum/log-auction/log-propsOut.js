$(function(){
    ...
    var logPropsOutViewModel = function () {
        ...
        this.getServerIdChange      = ko.computed(function () {
            if (indexModel.zeusidChange() == "" && $(".pageContainer").attr("data-url") == "/static/pages-html/log-sum/log-auction/log-propsOut.html") {
    ...
    var newLogModel                 = new logPropsOutViewModel();
    ko.applyBindings(newLogModel, $("#logPropsOut").get(0));
    ...
    var pageClick = function (pageclickednumber) {
        ...
        $.ajax({
            type: "get",
            async: true,
            url: "/gameLog/getCancelAuctionLogPage",
            data: jsonData,
            dataType: "json",
            error: function () {
                layer.close(index);
            ...
            success: function (data) {
                console.log("道具下架：",data)
                layer.close(index);
                if (data.code == 200) {
                    var tabDom = $("#propsOut_tbody").empty();
                    ...
                    if (data.info.rows) {
                        ...
                        console.log("log-propsOut data.info.rows is no data.");
                    }
    ...
    var roleForm = $("#logPropsOutForm").Validform({
