$(function(){
    ...
    var logAcceptTaskViewModel = function () {
        this.dataList               = ko.observableArray();
        this.taskId                 = ko.observable("");
        this.roleId                 = ko.observable("");
        this.getServerIdChange      = ko.computed(function () {
            if (indexModel.zeusidChange() == "" && $(".pageContainer").attr("data-url") == "/static/pages-html/log-sum/log-task/log-acceptTask.html") {
    ...
    var newLogModel = new logAcceptTaskViewModel();
    ko.applyBindings(newLogModel, $("#acceptTask").get(0));
    ...
    var pageClick = function (pageclickednumber) {
        ...
        $.ajax({
            type: "get",
            async: true,
            url: "/gameLog/getTaskAcceptLogPage",
            data: jsonData,
            dataType: "json",
            error: function () {
                layer.close(index);
            ...
            success: function (data) {
                console.log("任务接取日志：",data)
                layer.close(index);
                if (data.code == 200) {
                    var tabDom = $("#acceptTask_tbody").empty();
                    ...
                    if (data.info.rows) {
                        ...
                        console.log("log-acceptTask data.info.rows is no data.");
                    }
    ...
    var roleForm = $("#acceptTaskForm").Validform({
