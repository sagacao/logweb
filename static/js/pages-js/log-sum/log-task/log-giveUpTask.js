$(function(){
    ...
    var logGiveUpTaskViewModel = function () {
        this.dataList               = ko.observableArray();
        this.taskId                 = ko.observable("");
        this.roleId                 = ko.observable("");
        this.getServerIdChange      = ko.computed(function () {
            if (indexModel.zeusidChange() == "" && $(".pageContainer").attr("data-url") == "/static/pages-html/log-sum/log-task/log-giveUpTask.html") {
    ...
    var newLogModel = new logGiveUpTaskViewModel();
    ko.applyBindings(newLogModel, $("#giveUpTask").get(0));
    ...
    var pageClick = function (pageclickednumber) {
        ...
        $.ajax({
            ...
            success: function (data) {
                console.log("任务放弃日志：",data)
                layer.close(index);
                if (data.code == 200) {
                    var tabDom = $("#giveUpTask_tbody").empty();
                    ...
                    if (data.info.rows) {
                        ...
                        console.log("log-giveUpTask data.info.rows is no data.");
                    }
    ...
    var roleForm = $("#giveUpTaskForm").Validform({
