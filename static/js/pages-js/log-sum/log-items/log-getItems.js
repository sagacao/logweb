$(function(){
	...
	var logGetItemsViewModel = function(){
		this.dataList				= ko.observableArray();
		this.itemId					= ko.observable("");
		this.roleId					= ko.observable("");
		this.getServerIdChange		= ko.computed(function(){
			if (indexModel.zeusidChange() == "" && $(".pageContainer").attr("data-url") == "/static/pages-html/log-sum/log-items/log-getItems.html"){
	...
	var newLogModel = new logGetItemsViewModel();
	ko.applyBindings(newLogModel,document.getElementById("getItems"));
	...
	var pageClick = function( pageclickednumber ){
		...
		$.ajax({
			type : "get",
			async:true,
			url:"/gameLog/getItemObtainLogPage",
			data : jsonData,
			dataType : "json",
			error:function(){
				layer.close(index); 
			...
			success : function(data){
				console.log("物品获取日志：",data)
				layer.close(index); 
				if( data.code == 200 ){
					var tabDom = $("#getItems_tbody").empty();
					newLogModel.dataList.removeAll();
					if (data.info.rows) {
						for (var i = 0; i < data.info.rows.length; i++) {
							//data.info.rows[i].typeName = indexModel.itemObtainMap()[data.info.rows[i].type];
						...
						console.log("log-getItems data.info.rows is no data.");
					}
	...
	var roleForm = $("#getItemsForm").Validform({
